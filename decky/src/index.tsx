...
function ConfigDetails({ status }: { status: Status | null }) {
  ...
  return (
    <div style={{ fontSize: "12px", opacity: 0.8, lineHeight: 1.35 }}>
      ...
      <div>Volume path: {formatInitiator(initiator)} to {audioTarget}</div>
...
function Content() {
  ...
  const inputInactiveDelay = configValue(
    ...
    configValue(status, "INPUT_AWAY_SUSPEND_DELAY_SECONDS", "60"),
  );
      ...
      <PanelSection title="Configuration">
        ...
        <PanelSectionRow>
          <DropdownItem
            ...
            description="Auto uses the SteamOS adapter's current logical address. Select TV only for receivers that reject playback-device volume."
        ...
        <PanelSectionRow>
          <ToggleField
            ...
            description="For LG TVs that render their own audio and ignore standard CEC volume keys."
            checked={simplinkAck}
            disabled={busy}
